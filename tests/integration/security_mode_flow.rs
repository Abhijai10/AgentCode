...
/// G5 real dependency manifest: the audit reads the project's actual
/// Cargo.lock, parses pinned versions, and flags a known-vulnerable
/// dependency honestly (a clean dependency is never flagged).
#[test]
fn security_audit_discovers_real_dependency_manifest_and_flags_known_vulnerable() {
    let runtime = temp_root("runtime-manifest");
    let project = temp_root("project-manifest");
    let _ = fs::remove_dir_all(&runtime);
    let _ = fs::remove_dir_all(&project);
    fs::create_dir_all(&runtime).unwrap();
    create_security_fixture(&project);

    // Real Cargo.lock with one known-vulnerable pinned dependency
    // (lodash is npm-flavored, so use the Cargo-style form the parser
    // accepts plus a package.json variant for the same project to prove
    // priority: Cargo.lock wins over package.json when both exist).
    fs::write(
    ...
    let create = request_via_ipc(
        ...
        &listener,
        &mut daemon,
        json!({"id":"cs","command":"ConversationCreate","project_path": project_path, "mode":"SECURITY","title":"Manifest Audit"}),
    );
    assert_eq!(create["ok"], true, "create: {create}");
    let cid = create["conversation_id"].as_str().unwrap().to_string();
    ...
    assert_eq!(audit["ok"], true, "audit: {audit}");
    let audit_body = &audit["audit"];

    // The known-vulnerable elliptic 6.4.1 (Cargo.lock) must be flagged.
    let elliptic = finding_by_root(&audit_body["findings"], "builtin-dependency-elliptic-6.4.1");
    let root_causes: Vec<String> = audit_body["findings"]
        .as_array()
        .map(|f| {
            f.iter()
                .map(|x| x["root_cause"].as_str().unwrap_or("").to_string())
                .collect::<Vec<_>>()
        })
        .unwrap_or_default();
    assert!(
        elliptic.is_some(),
        "elliptic 6.4.1 from Cargo.lock must be flagged: {root_causes:?}"
    );
    ...
    // Cargo.lock has priority over package.json: lodash (package.json)
    // is NOT analyzed in this run — proving the priority is real, not
    // that both files were merged.
    let lodash = finding_by_root(&audit_body["findings"], "builtin-dependency-lodash");
    assert!(
        lodash.is_none(),
        "package.json must not be read when Cargo.lock exists (priority): {root_causes:?}"
    );

    // The finding is persisted: re-listing findings through the IPC
    // command (the public production path) shows the manifest finding.
    let findings = security_command(
        &server,
        &listener,
        &mut daemon,
        &cid,
    ...
    assert_eq!(findings["ok"], true, "findings: {findings}");
    let persisted_roots: Vec<String> = findings["result"]["findings"]
        .as_array()
        .map(|f| {
            f.iter()
                .map(|x| x["root_cause"].as_str().unwrap_or("").to_string())
...
/// G5 audit-depth policies: quick depth runs the fast scanner set only;
/// adversarial depth attempts ZAP DAST only when the scope authorizes an
/// active localhost target — otherwise ZAP is recorded UNAVAILABLE, never
/// faked.
#[test]
fn security_audit_depth_policies_are_distinct_and_dast_requires_authorization() {
    let runtime = temp_root("runtime-depth");
    let project = temp_root("project-depth");
    let _ = fs::remove_dir_all(&runtime);
    let _ = fs::remove_dir_all(&project);
    fs::create_dir_all(&runtime).unwrap();
    create_security_fixture(&project);
    ...
    let create = request_via_ipc(
        ...
        &listener,
        &mut daemon,
        json!({"id":"cs","command":"ConversationCreate","project_path": project_path, "mode":"SECURITY","title":"Depth Audit"}),
    );
    assert_eq!(create["ok"], true, "create: {create}");
    let cid = create["conversation_id"].as_str().unwrap().to_string();
    ...
    // Quick depth: only the fast content scanners are attempted.
    let quick = security_command(
        &server,
        &listener,
        &mut daemon,
        &cid,
    ...
    assert_eq!(quick["ok"], true, "quick: {quick}");
    let quick_body = &quick["audit"];
    ...
    for slow in ["Osv", "Trivy", "Checkov", "Zap"] {
        assert!(
            !quick_attempted.iter().any(|a| a == slow),
            "quick depth must not attempt {slow}: {quick_attempted:?}"
        );
    }

    // Adversarial depth with a READ-ONLY scope: no authorized active
    // target exists, so ZAP must appear as unavailable (not silently
    // skipped, not faked) — the honest DAST gate.
    let adversarial = security_command(
        &server,
        &listener,
        &mut daemon,
        &cid,
        "SecurityAudit",
        json!({"depth": "adversarial"}),
    );
    assert_eq!(adversarial["ok"], true, "adversarial: {adversarial}");
    let adversarial_body = &adversarial["audit"];
    ...
    assert!(
        !zap_rows.is_empty(),
        "adversarial depth must attempt ZAP (recorded honestly): {}",
    ...
    // UNAVAILABLE/MISCONFIGURED — the authorization gate is real.
    assert!(
        zap_rows.iter().all(|z| z["availability"] != "Available"),
        "read-only scope must not yield an executed ZAP run: {zap_rows:?}"
    );

    // Full depth: the complete static set is attempted (adapter names
    // appear in either availability list — executed or honestly failed).
    let full = security_command(
        &server,
        &listener,
        &mut daemon,
        &cid,
    ...
    assert_eq!(full["ok"], true, "full: {full}");
    assert_eq!(full["audit"]["audit_depth"], "full");
